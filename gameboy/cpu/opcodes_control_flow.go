...
var OpCodesControlFlow = map[uint8]OpCode{
	...
	0xc2: NewOpCode(0xc2, "JP NZ,u16", 3, 16, []func(cpu *CPU){
		func(cpu *CPU) { lsb = cpu.bus.BusRead(cpu.pc); cpu.pc++ },
		func(cpu *CPU) { msb = cpu.bus.BusRead(cpu.pc); cpu.pc++; stop = cpu.regs.getFlag(FLAG_ZERO_Z_BIT) },
		func(cpu *CPU) { cpu.pc = utils.ToUint16(lsb, msb) },
	}),
	...
	0xd2: NewOpCode(0xd2, "JP NC,u16", 3, 16, []func(cpu *CPU){
		func(cpu *CPU) { lsb = cpu.bus.BusRead(cpu.pc); cpu.pc++ },
		func(cpu *CPU) { msb = cpu.bus.BusRead(cpu.pc); cpu.pc++; stop = cpu.regs.getFlag(FLAG_CARRY_C_BIT) },
		func(cpu *CPU) { cpu.pc = utils.ToUint16(lsb, msb) },
	}),
	...
	0x20: NewOpCode(0x20, "JR NZ,i8", 2, 12, []func(cpu *CPU){
		func(cpu *CPU) { e = cpu.bus.BusRead(cpu.pc); cpu.pc++; stop = cpu.regs.getFlag(FLAG_ZERO_Z_BIT) },
	...
	0x30: NewOpCode(0x30, "JR NC,i8", 2, 12, []func(cpu *CPU){
		func(cpu *CPU) { e = cpu.bus.BusRead(cpu.pc); cpu.pc++; stop = cpu.regs.getFlag(FLAG_CARRY_C_BIT) },
		func(cpu *CPU) { cpu.pc = uint16(int(cpu.pc) + int(int8(e))) },
	}),
