...
var OpCodes16bitLoadGenerated = map[uint8]OpCode{
	...
	0x01: NewOpCode(0x01, "LD BC,u16", 3, 12, []func(cpu *CPU){
		...
		func(cpu *CPU) { msb = cpu.bus.Read(cpu.pc); cpu.pc++; cpu.regs.setBC(utils.ToUint16(lsb, msb)) },
	}),
	0x11: NewOpCode(0x11, "LD DE,u16", 3, 12, []func(cpu *CPU){
		func(cpu *CPU) { lsb = cpu.bus.Read(cpu.pc); cpu.pc++ },
		func(cpu *CPU) { msb = cpu.bus.Read(cpu.pc); cpu.pc++; cpu.regs.setDE(utils.ToUint16(lsb, msb)) },
	}),
	0x21: NewOpCode(0x21, "LD HL,u16", 3, 12, []func(cpu *CPU){
		func(cpu *CPU) { lsb = cpu.bus.Read(cpu.pc); cpu.pc++ },
		func(cpu *CPU) { msb = cpu.bus.Read(cpu.pc); cpu.pc++; cpu.regs.setHL(utils.ToUint16(lsb, msb)) },
	}),
	0x31: NewOpCode(0x31, "LD SP,u16", 3, 12, []func(cpu *CPU){
		func(cpu *CPU) { lsb = cpu.bus.Read(cpu.pc); cpu.pc++ },
		func(cpu *CPU) { msb = cpu.bus.Read(cpu.pc); cpu.pc++; cpu.sp = utils.ToUint16(lsb, msb) },
	}),
	...
	0x08: NewOpCode(0x08, "LD (u16),SP", 3, 20, []func(cpu *CPU){
		...
		func(cpu *CPU) { cpu.bus.Write(utils.ToUint16(lsb, msb), utils.Lsb(cpu.sp)) },
		func(cpu *CPU) { cpu.bus.Write(utils.ToUint16(lsb, msb)+1, utils.Msb(cpu.sp)) },
	}),
	...
	0xf8: NewOpCode(0xf8, "LD HL,SP+i8", 2, 12, []func(cpu *CPU){
		...
		func(cpu *CPU) { cpu.regs.setHL(addSigned8(cpu, cpu.sp, e)) },
	}),
	// Push to stack
