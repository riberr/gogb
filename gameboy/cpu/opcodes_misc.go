...
var OpCodesMisc = map[uint8]OpCode{
	...
	// Disable interrupts
	0xf3: NewOpCode(0xf3, "DI", 1, 4, []func(cpu *CPU){func(cpu *CPU) { /*cpu.interrupts.DisableIME()*/ cpu.interrupts.SetIMEEnabling(false) }}),

	// Enable interrupts
	0xfb: NewOpCode(0xfb, "EI", 1, 4, []func(cpu *CPU){func(cpu *CPU) { /*cpu.interrupts.EnableIME()*/ cpu.interrupts.SetIMEEnabling(true) }}),
